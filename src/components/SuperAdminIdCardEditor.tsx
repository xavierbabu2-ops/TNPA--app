...
interface SuperAdminIdCardEditorProps {
  ...
  onAddRegistration?: (newReg: MemberRegistration) => void;
...
export default function SuperAdminIdCardEditor({
  ...
  const [newCardCategory, setNewCardCategory] = useState<"member" | "official">("member");
  const [newCardName, setNewCardName] = useState("");
  const [newCardFather, setNewCardFather] = useState("");
  const [newCardRole, setNewCardRole] = useState("உறுப்பினர் (Member)");
  const [newCardDistrict, setNewCardDistrict] = useState("மதுரை");
  const [newCardPhone, setNewCardPhone] = useState("");
  const [newCardBlood, setNewCardBlood] = useState("O+");
  const [newCardRegNo, setNewCardRegNo] = useState("TNMDUJ-NEW-" + Math.floor(1000 + Math.random() * 9000));
  const [newCardPhoto, setNewCardPhoto] = useState("https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=200&h=200");
  const [newBrandName, setNewBrandName] = useState("TNPA² Gold Standard Edition 2026");
  const [brandImageGenerating, setBrandImageGenerating] = useState(false);
  const [brandAssetBanner, setBrandAssetBanner] = useState("");

  const handleCreateNewIdCard = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isSuperAdmin) {
      alert(lang === "ta" ? "அனுமதி மறுக்கப்பட்டது: சூப்பர் அட்மின் மட்டுமே புதிய ஐடி கார்டு உருவாக்க முடியும்." : "Access Denied: Only Super Admin can create new ID cards.");
      return;
    }

    if (!newCardName.trim() || !newCardRegNo.trim()) {
      alert(lang === "ta" ? "தயவுசெய்து பெயர் மற்றும் பதிவு எண்ணை உள்ளிடவும்." : "Please enter member name and registration number.");
      return;
    }
    ...
    const newReg: MemberRegistration = {
      ...
      regNumber: newCardRegNo,
      photoUrl: newCardPhoto,
      dob: "1990-01-01",
      gender: "ஆண் / Male",
      aadhaar: "9876 5432 1098",
      address: `${newCardDistrict}, தமிழ்நாடு`,
      experienceYears: 5,
      status: "approved",
      createdAt: new Date().toISOString(),
      cardType: newCardCategory,
      designation: newCardRole,
      brandEdition: newBrandName,
      brandBanner: brandAssetBanner || customBgUrl
    };

    if (onAddRegistration) {
      onAddRegistration(newReg);
    } else if (onUpdateRegistration) {
      onUpdateRegistration(newReg);
    }

    onAddAuditLog("Super Admin Created New ID Card", `Super Admin successfully generated new ${newCardCategory === "official" ? "Official Leader" : "Member"} ID card for ${newCardName} (${newCardRegNo}) under brand: ${newBrandName}`);
    alert(lang === "ta" ? `✓ புதிய ${newCardCategory === "official" ? "பொறுப்பாளர்" : "உறுப்பினர்"} அடையாள அட்டை சூப்பர் அட்மினால் வெற்றிகரமாக உருவாக்கப்பட்டு கணினியில் சேர்க்கப்பட்டது!` : `✓ New ${newCardCategory} ID card successfully created and issued by Super Admin!`);
    
    setStudioTab("edit_existing");
  };

  const handleGenerateBrandAssets = () => {
    ...
    setTimeout(() => {
      ...
      const randomBanner = palettes[Math.floor(Math.random() * palettes.length)];
      setBrandAssetBanner(randomBanner);
      setBrandImageGenerating(false);
      onAddAuditLog("Super Admin Brand Asset Generation", `Generated new AI brand theme assets & background textures for brand: ${newBrandName}`);
      alert(lang === "ta" ? "✨ புதிய பிராண்ட் கலை மற்றும் பின்னணி படங்கள் சூப்பர் அட்மின் ஸ்டுடியோ மூலம் வெற்றிகரமாக உருவாக்கப்பட்டன!" : "✨ New AI brand assets & background graphics successfully generated by Super Admin Studio!");
    }, 1000);
  };
  ...
  const [customGovtSealUrl, setCustomGovtSealUrl] = useState("");
  const [customLeader1Url, setCustomLeader1Url] = useState("");
  const [customLeader2Url, setCustomLeader2Url] = useState("");
  ...
  return (
    <div className="space-y-6 text-left animate-[fadeIn_0.3s_ease-out]">
      ...
      {/* STUDIO TAB SWITCHER */}
      <div className="flex bg-stone-200/80 p-1.5 rounded-2xl max-w-lg gap-2 border border-stone-300">
        <button
          onClick={() => setStudioTab("edit_existing")}
          className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 ${
            studioTab === "edit_existing"
              ? "bg-[#b91c1c] text-white shadow-md"
              : "text-stone-700 hover:bg-white/60"
          }`}
        >
          <Edit3 className="w-4 h-4" />
          <span>{lang === "ta" ? "இருக்கும் அட்டைகளைத் திருத்துதல்" : "Edit Existing Cards"}</span>
        </button>
        <button
          onClick={() => setStudioTab("create_new")}
          className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 ${
            studioTab === "create_new"
              ? "bg-amber-500 text-stone-950 shadow-md"
              : "text-stone-700 hover:bg-white/60"
          }`}
        >
          <Plus className="w-4 h-4" />
          <span>{lang === "ta" ? "✨ புதிய ஐடி கார்டு & பிராண்ட் ஸ்டுடியோ" : "✨ New ID Card & Brand Studio"}</span>
        </button>
      </div>

      {studioTab === "create_new" ? (
        /* CREATE NEW ID CARD & BRAND STUDIO */
        <div className="bg-white border border-stone-200 rounded-3xl p-6 md:p-8 shadow-sm space-y-6 animate-[fadeIn_0.3s_ease-out]">
          <div className="border-b pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <span className="text-[10px] font-black uppercase text-amber-600 bg-amber-50 px-2 py-0.5 rounded">
                {lang === "ta" ? "சூப்பர் அட்மின் பிரத்யேக உருவாக்கம்" : "Super Admin Exclusive Creation"}
              </span>
              <h4 className="text-lg font-black text-stone-900 mt-1">
                {lang === "ta" ? "🎴 புதிய உறுப்பினர் அல்லது பொறுப்பாளர் அடையாள அட்டை உருவாக்குதல்" : "Create New Member or Official Leader ID Card"}
              </h4>
              <p className="text-xs text-stone-600">
                {lang === "ta"
                  ? "புதிய பிராண்ட் மற்றும் கலை வேலைப்பாடுகளுடன் கூடிய புதிய அடையாள அட்டையை உடனே உருவாக்கி கணினியில் இணைக்கவும்."
                  : "Design and issue a brand new ID card with custom branding, AI brand assets, and instant database registration."}
              </p>
            </div>
          </div>

          <form onSubmit={handleCreateNewIdCard} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  {lang === "ta" ? "அடையாள அட்டை வகை (Card Category)" : "Card Category"}
                </label>
                <select
                  value={newCardCategory}
                  onChange={(e) => setNewCardCategory(e.target.value as "member" | "official")}
                  className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs font-bold text-stone-900"
                >
                  <option value="member">{lang === "ta" ? "உறுப்பினர் அடையாள அட்டை (Member ID Card)" : "Member ID Card"}</option>
                  <option value="official">{lang === "ta" ? "பொறுப்பாளர் அடையாள அட்டை (Official Leader ID Card)" : "Official Leader ID Card"}</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  {lang === "ta" ? "உறுப்பினர் / பொறுப்பாளர் பெயர் (Name)" : "Name"}
                </label>
                <input
                  type="text"
                  value={newCardName}
                  onChange={(e) => setNewCardName(e.target.value)}
                  placeholder={lang === "ta" ? "எ.கா: திரு. ஆர். முருகன்" : "e.g. R. Murugan"}
                  className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs font-bold text-stone-900"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={lang === "ta" ? "தந்தை பெயர்" : "Father Name"}
                  className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs font-bold text-stone-900"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  {lang === "ta" ? "பதவி / பொறுப்பு (Role / Designation)" : "Role / Designation"}
                </label>
                <input
                  type="text"
                  value={newCardRole}
                  onChange={(e) => setNewCardRole(e.target.value)}
                  placeholder={lang === "ta" ? "எ.கா: மாவட்ட தலைவர் / உறுப்பினர்" : "e.g. District President / Member"}
                  className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs font-bold text-stone-900"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={newCardDistrict}
                  onChange={(e) => setNewCardDistrict(e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs font-bold text-stone-900"
                  required
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  {lang === "ta" ? "பதிவு எண் (Registration Number)" : "Registration No"}
                </label>
                <input
                  type="text"
                  value={newCardRegNo}
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  {lang === "ta" ? "புகைப்பட URL (Photo URL / Upload)" : "Photo URL"}
                </label>
                <input
                  type="text"
                  value={newCardPhoto}
            ...
            {/* BRAND & AI ASSET STUDIO SECTION */}
            <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h5 className="font-black text-stone-900 text-sm flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-amber-600" />
                  <span>{lang === "ta" ? "✨ புதிய பிராண்ட் & AI கலை உருவாக்கம் (Brand & Asset Generator)" : "✨ Brand & AI Asset Generator Studio"}</span>
                </h5>
                <span className="text-[10px] font-bold text-stone-500">{lang === "ta" ? "பிரத்யேக சூப்பர் அட்மின் ஆற்றல்" : "Super Admin AI Power"}</span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-stone-700 mb-1">
                    {lang === "ta" ? "புதிய பிராண்ட் பெயர் / பதிப்பு (Brand Title)" : "New Brand Title / Edition"}
                  </label>
                  <input
                    type="text"
                    value={newBrandName}
                ...
                <div className="flex items-end">
                  <button
                    ...
                    disabled={brandImageGenerating}
                    className="w-full py-2.5 px-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-stone-950 font-black text-xs rounded-xl shadow transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {brandImageGenerating ? (
                      <RefreshCw className="w-4 h-4 animate-spin" />
                    ) : (
                      <Sparkles className="w-4 h-4" />
                    )}
                    <span>{lang === "ta" ? "✨ புதிய பிராண்ட் படங்களை உருவாக்கு (Generate Assets)" : "✨ Generate Brand AI Assets"}</span>
                  </button>
                </div>
              </div>

              {brandAssetBanner && (
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-4">
                  <img src={brandAssetBanner} alt="Brand Asset" className="w-16 h-16 object-cover rounded-lg border border-amber-300" />
                  <div>
                    <div className="text-xs font-black text-amber-900">{newBrandName}</div>
                    <div className="text-[10px] text-amber-700">✓ AI Brand background & badge assets synthesized successfully.</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              <button
                type="button"
                onClick={() => setStudioTab("edit_existing")}
                className="px-5 py-2.5 bg-stone-200 hover:bg-stone-300 text-stone-800 font-bold text-xs rounded-xl transition-all"
              >
                {lang === "ta" ? "ரத்து செய்" : "Cancel"}
              </button>
              <button
                type="submit"
                className="px-6 py-2.5 bg-[#b91c1c] hover:bg-red-800 text-white font-black text-xs rounded-xl shadow-lg transition-all flex items-center gap-2 cursor-pointer"
              >
                <Save className="w-4 h-4" />
                <span>{lang === "ta" ? "🎴 புதிய அடையாள அட்டையை சேமித்து வெளியிடு (Issue New ID Card)" : "Issue & Save New ID Card"}</span>
              </button>
            </div>
          </form>
        </div>
          ...
          <div className="bg-white border border-stone-200 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-3 border-t space-y-3">
                ...
                {/* Back Card Images: Govt Seal & Leaders Upload */}
                <div className="pt-3 border-t grid grid-cols-1 md:grid-cols-3 gap-2">
                  {/* Govt Seal Upload */}
                  <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-2xl space-y-2 text-center">
                    <span className="text-[10px] font-black text-emerald-900 block">
                      {lang === "ta" ? "அரசு முத்திரை / சின்னம்" : "Govt Seal / Emblem"}
                    </span>
                    {customGovtSealUrl ? (
                      <div className="w-12 h-12 mx-auto rounded-full overflow-hidden border-2 border-emerald-600 bg-white flex items-center justify-center">
                        <img src={customGovtSealUrl} alt="Seal" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="w-12 h-12 mx-auto rounded-full bg-emerald-100 flex items-center justify-center text-emerald-800 text-xs font-bold">
                        🏛️ Seal
                      </div>
                    )}
                    <label className="inline-block px-3 py-1.5 bg-emerald-700 text-white rounded-lg text-[10px] font-bold cursor-pointer hover:bg-emerald-800">
                      {lang === "ta" ? "முத்திரை மாற்று" : "Change Seal"}
                      <input type="file" accept="image/*" onChange={(e) => handleGalleryFileSelect(e, setCustomGovtSealUrl)} className="hidden" />
                    </label>
                  </div>

                  {/* Leader 1 Photo Upload */}
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl space-y-2 text-center">
                    <span className="text-[10px] font-black text-amber-900 block">
                      {lang === "ta" ? "தலைவர் 1 புகைப்படம்" : "Leader 1 Photo"}
                    </span>
                    {customLeader1Url ? (
                      <div className="w-10 h-12 mx-auto rounded overflow-hidden border border-amber-600 bg-white flex items-center justify-center">
                        <img src={customLeader1Url} alt="L1" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="w-10 h-12 mx-auto rounded bg-amber-100 flex items-center justify-center text-amber-800 text-[9px] font-bold">
                        Leader 1
                      </div>
                    )}
                    <label className="inline-block px-3 py-1.5 bg-amber-600 text-white rounded-lg text-[10px] font-bold cursor-pointer hover:bg-amber-700">
                      {lang === "ta" ? "தலைவர் 1 மாற்று" : "Change L1"}
                      <input type="file" accept="image/*" onChange={(e) => handleGalleryFileSelect(e, setCustomLeader1Url)} className="hidden" />
                    </label>
                  </div>

                  {/* Leader 2 Photo Upload */}
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl space-y-2 text-center">
                    <span className="text-[10px] font-black text-amber-900 block">
                      {lang === "ta" ? "தலைவர் 2 புகைப்படம்" : "Leader 2 Photo"}
                    </span>
                    {customLeader2Url ? (
                      <div className="w-10 h-12 mx-auto rounded overflow-hidden border border-amber-600 bg-white flex items-center justify-center">
                        <img src={customLeader2Url} alt="L2" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="w-10 h-12 mx-auto rounded bg-amber-100 flex items-center justify-center text-amber-800 text-[9px] font-bold">
                        Leader 2
                    ...
                    <label className="inline-block px-3 py-1.5 bg-amber-600 text-white rounded-lg text-[10px] font-bold cursor-pointer hover:bg-amber-700">
                      {lang === "ta" ? "தலைவர் 2 மாற்று" : "Change L2"}
                      <input type="file" accept="image/*" onChange={(e) => handleGalleryFileSelect(e, setCustomLeader2Url)} className="hidden" />
                    </label>
                  </div>
                </div>
